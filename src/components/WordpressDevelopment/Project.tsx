        ...
        {data?.map((item: any, index: number) => (
          <Link href={`/projects/${item?.slug}/`} key={index}>
            <div className="border border-sky-500 p-5 group relative h-[300px] rounded-xl overflow-hidden shadow-md hover:shadow-2xl">
              ...
              <div className=" absolute bottom-5 right-7  z-10 group-hover:translate-x-2 border border-sky-500 group-hover:bg-sky-500 duration-200 rounded-full p-4 h-8 w-8 grid justify-center content-center">
                <MdOutlineKeyboardArrowRight className="text-sky-500 h-6 w-6 group-hover:text-white duration-200" />
              </div>
            </div>
          </Link>
        ))}
